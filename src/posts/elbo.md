...
At this point, since we have an expectation, we can estimate it using the Monte Carlo method\footnote{which is basically just sampling it a buncha times and averaging instead of summing over all possible latents} therefore it is now _tractable_, given that sampling from $q(z \mid x)$ (where you got your latent variables from) is also affordable. However, we still have a problem.

### Biasing an Unbiased Estimator
At the very core, we want to compute $-\log p^\theta(x)$. But we've established that evaluating $p^\theta(x)$ exactly is _intractable_, but we have rewritten it as an expectation:

$$
p^\theta(x)
=
\mathbb E_{q(z\mid x)}
\left[\frac{p^\theta(x\mid z)p(z)}{q(z\mid x)}\right].
$$

We can approximate this expectation using a Monte Carlo sample average. Draw $K$ independent samples $z_k\sim q(z\mid x)$, then compute

$$
\widehat p_K^\theta(x)
=
\frac1K\sum_{k=1}^{K}
\frac{p^\theta(x\mid z_k)p(z_k)}{q(z_k\mid x)}.
$$

Which makes it _tractable_. Now, Monte Carlo sample average here is an **unbiased estimator.** An unbiased estimator is an estimator for  that if you repeated the sampling many times, your estimates would average out to the true value\footnote{if this sentence is hard to read for you, i suggest learning Statistical Inference™}. Now the problem is, there is a $\log$ term outside of that estimator

$$
-\log(\widehat p_K^\theta(x))
$$

Had we computed the exact $p^\theta(x)$, taking its negative log would give us the exact objective. But we instead have a random estimate $\widehat p_K^\theta(x)$. Applying the negative log generally introduces bias: on average, we overestimate the true negative log-likelihood, even though individual estimates may fall above or below it.

## Lower Bounding
We mitigate this problem by opting to optimize something else that is _not exactly_ the negative log likelihood, but can be estimated _without bias_. By Jensen's Inequality, since $\log$ is concave,
...
_**That final right-hand side is the negative ELBO (NELBO)!**_ (ELBO itself is NELBO without the negative, the inequality right before it) We minimize it using yet another Monte Carlo estimates. When we do that, we'd get a true negative log likelihood that's always at least as low as the negative ELBO.